...
dischi = ['disk1', 'disk5']
versioni = ['V1', 'V2', 'V3']
metrica = 'Mean'  # Cambia in 'Total' per plottare Erosion/Dilation Total
...
fig, axes = plt.subplots(len(dischi), len(dimensioni), figsize=(11.69, 8.27), sharex=True, sharey=True)
...
for row, disco in enumerate(dischi):
    for col, dimensione in enumerate(dimensioni):
        ...
        for v in versioni:
            file_path = f'build/Release/results/{dimensione}_{disco}/csv_speedup_{v}_{dimensione}_{disco}.csv'
            df = pd.read_csv(file_path)

            for op in operazioni:
                col_key = f'{op[0]}_{metrica}'  # E_Mean o E_Total, ecc.
                label = f'{op} ({v})'
...
# Titolo e layout
plt.suptitle(f'Confronto Erosion/Dilation {metrica} - Versioni V1, V2, V3', fontsize=14)
